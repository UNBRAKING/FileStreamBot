# from os import environ as env
# from dotenv import load_dotenv

# load_dotenv()

# class Telegram:
#     API_ID = int(env.get("API_ID"))
#     API_HASH = str(env.get("API_HASH"))
#     BOT_TOKEN = str(env.get("BOT_TOKEN"))
#     OWNER_ID = int(env.get('OWNER_ID', '7978482443'))
#     WORKERS = int(env.get("WORKERS", "6"))  # 6 workers = 6 commands at once
#     DATABASE_URL = str(env.get('DATABASE_URL'))
#     UPDATES_CHANNEL = str(env.get('UPDATES_CHANNEL', "Telegram"))
#     SESSION_NAME = str(env.get('SESSION_NAME', 'FileStream'))
#     FORCE_SUB_ID = env.get('FORCE_SUB_ID', None)
#     FORCE_SUB = env.get('FORCE_UPDATES_CHANNEL', True)
#     FORCE_SUB = True if str(FORCE_SUB).lower() == "true" else False
#     SLEEP_THRESHOLD = int(env.get("SLEEP_THRESHOLD", "60"))
#     FILE_PIC = env.get('FILE_PIC', "https://graph.org/file/5bb9935be0229adf98b73.jpg")
#     START_PIC = env.get('START_PIC', "https://graph.org/file/290af25276fa34fa8f0aa.jpg")
#     VERIFY_PIC = env.get('VERIFY_PIC', "https://graph.org/file/736e21cc0efa4d8c2a0e4.jpg")
#     MULTI_CLIENT = False
#     FLOG_CHANNEL = int(env.get("FLOG_CHANNEL", None))   # Logs channel for file logs
#     ULOG_CHANNEL = int(env.get("ULOG_CHANNEL", None))   # Logs channel for user logs
#     MODE = env.get("MODE", "primary")
#     SECONDARY = True if MODE.lower() == "secondary" else False
#     AUTH_USERS = list(set(int(x) for x in str(env.get("AUTH_USERS", "")).split()))

# class Server:
#     PORT = int(env.get("PORT", 8080))
#     BIND_ADDRESS = str(env.get("BIND_ADDRESS", "0.0.0.0"))
#     PING_INTERVAL = int(env.get("PING_INTERVAL", 1200))
#     HAS_SSL = str(env.get("HAS_SSL", "0").lower()) in ("1", "true", "t", "yes", "y")
#     NO_PORT = True  # Force this to True for Heroku
    
#     # Get FQDN from env or use Heroku app name
#     FQDN = env.get("FQDN")
#     if not FQDN or FQDN in ["0.0.0.0", "localhost", "127.0.0.1"]:
#         APP_NAME = env.get("HEROKU_APP_NAME")  # Make sure this is set in Config Vars
#         if APP_NAME:
#             FQDN = f"{APP_NAME}.herokuapp.com"
#         else:
#             FQDN = BIND_ADDRESS

#     URL = "http{}://{}/".format(
#         "s" if HAS_SSL else "", FQDN)


from os import environ as env
from dotenv import load_dotenv

# Load environment variables from .env file
load_dotenv()

class Telegram:
    # Telegram API details
    API_ID = int(env.get("API_ID"))
    API_HASH = str(env.get("API_HASH"))
    BOT_TOKEN = str(env.get("BOT_TOKEN"))
    OWNER_ID = int(env.get('OWNER_ID', '7978482443'))
    WORKERS = int(env.get("WORKERS", "6"))  # 6 workers = 6 commands at once

    # Database and Channels
    DATABASE_URL = str(env.get('DATABASE_URL'))
    UPDATES_CHANNEL = str(env.get('UPDATES_CHANNEL', "Telegram"))
    SESSION_NAME = str(env.get('SESSION_NAME', 'FileStream'))
    FORCE_SUB_ID = env.get('FORCE_SUB_ID', None)
    FORCE_SUB = env.get('FORCE_UPDATES_CHANNEL', True)
    FORCE_SUB = True if str(FORCE_SUB).lower() == "true" else False

    # Misc Settings
    SLEEP_THRESHOLD = int(env.get("SLEEP_THRESHOLD", "60"))
    FILE_PIC = env.get('FILE_PIC', "https://graph.org/file/5bb9935be0229adf98b73.jpg")
    START_PIC = env.get('START_PIC', "https://graph.org/file/290af25276fa34fa8f0aa.jpg")
    VERIFY_PIC = env.get('VERIFY_PIC', "https://graph.org/file/736e21cc0efa4d8c2a0e4.jpg")
    MULTI_CLIENT = False

    # Log Channels
    FLOG_CHANNEL = int(env.get("FLOG_CHANNEL", None))
    ULOG_CHANNEL = int(env.get("ULOG_CHANNEL", None))

    # Mode Check
    MODE = env.get("MODE", "primary")
    SECONDARY = True if MODE.lower() == "secondary" else False

    # Authorized Users
    AUTH_USERS = list(set(int(x) for x in str(env.get("AUTH_USERS", "")).split()))


class Server:
    # Server settings
    PORT = int(env.get("PORT", 8080))
    BIND_ADDRESS = str(env.get("BIND_ADDRESS", "0.0.0.0"))
    PING_INTERVAL = int(env.get("PING_INTERVAL", 1200))
    HAS_SSL = str(env.get("HAS_SSL", "0").lower()) in ("1", "true", "t", "yes", "y")

    # Heroku ke liye hamesha NO_PORT True rakhna hai
    NO_PORT = True  

    # FQDN get karna, agar env me nahi diya to Heroku app name se banega
    FQDN = env.get("FQDN")
    if not FQDN or FQDN in ["0.0.0.0", "localhost", "127.0.0.1"]:
        APP_NAME = env.get("HEROKU_APP_NAME")
        if APP_NAME:
            FQDN = f"{APP_NAME}.herokuapp.com"
        else:
            FQDN = BIND_ADDRESS

    @classmethod
    def get_url(cls):
        """
        Server URL generate karega. 
        Agar NO_PORT True hai (Heroku case), to port nahi lagega.
        Nahi to port ke saath URL banega.
        """
        protocol = "https" if cls.HAS_SSL else "http"
        if cls.NO_PORT:
            return f"{protocol}://{cls.FQDN}/"
        else:
            return f"{protocol}://{cls.FQDN}:{cls.PORT}/"

    # URL ab dynamic banega via function
    URL = get_url.func(locals()["Server"])
